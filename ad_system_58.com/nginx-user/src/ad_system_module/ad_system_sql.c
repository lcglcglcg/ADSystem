...
int ad_system_sql_get_disease_code(ad_system_sql_t *p)
{
    ...
    char sql_str[1024] = {0};
    sprintf(sql_str, "select name, code from t_code_39_disease");
    fprintf(stdout, "%s\n", sql_str);
    return ad_system_sql_select(p, sql_str);
}
	...
	const char *find_name)
{
	...
	size += sprintf(&sql_str[size], " and account_id=%s", account_id);
	...
	size += sprintf(&sql_str[size], " and find_name like '%%%s%%", find_name);
...
//推广->修改文字创意
int ___ad_system_sql_set_creative_text(const char *creative_id, int status)
#if 0
    const char *account_id,
    const char *creative_id,
    int update_type,
	...
	const char *TEL)
{
    ...
    sprintf(sql_str, "INSERT INTO T_BRAND_CREATIVE ("
	    "CREATIVE_ID,"
	    "ACCOUNT_ID,"
	    "CAMPAIGN_ID,"
	    "ADGROUP_ID,"
	...
	const char *creative_id)
{
    ...
    size_t sql_size = sprintf(sql_str, "delete from T_BRAND_CREATIVE t where ");
    if (campaign_id)
	sql_size += sprintf(&sql_str[sql_size], " t.campaign_id='%s'", campaign_id);
    else if (group_id)
	sql_size += sprintf(&sql_str[sql_size], " t.adgroup_id='%s'", group_id);
	...
	const char *TEL)
{
    ...
    char comm[4] = {0};
    char sql_str[409600] = {0};
    size_t size = sprintf(sql_str, "update T_BRAND_CREATIVE set ");

    if (TITLE && *TITLE) {
	size += sprintf(&sql_str[size], "%sTITLE='%s'", comm, TITLE);
	strcpy(comm, ",");
    }

    if (DESCRIPTION && *DESCRIPTION) {
	size += sprintf(&sql_str[size], "%sDESCRIPTION='%s'", comm, DESCRIPTION);
	strcpy(comm, ",");
    }

    if (DISPLAY_URL && *DISPLAY_URL) {
	size += sprintf(&sql_str[size], "%sDESCRIPTION='%s'", comm, DISPLAY_URL);
	strcpy(comm, ",");
    }

    if (WEB_IM && *WEB_IM) {
	size += sprintf(&sql_str[size], "%sWEB_IM='%s'", comm, WEB_IM);
	strcpy(comm, ",");
    }

    if (IMG0 && *IMG0) {
	size += sprintf(&sql_str[size], "%sIMG1='%s'", comm, IMG0);
	strcpy(comm, ",");
    }

    if (IMG1 && *IMG1) {
	size += sprintf(&sql_str[size], "%sIMG2='%s'", comm, IMG1);
	strcpy(comm, ",");
    }

    if (IMG2 && *IMG2) {
	size += sprintf(&sql_str[size], "%sIMG3='%s'", comm, IMG2);
	strcpy(comm, ",");
    }

    if (IMG3 && *IMG3) {
	size += sprintf(&sql_str[size], "%sIMG4='%s'", comm, IMG3);
	strcpy(comm, ",");
    }

    if (IMG4 && *IMG4) {
	size += sprintf(&sql_str[size], "%sIMG5='%s'", comm, IMG4);
	strcpy(comm, ",");
    }

    if (IMG5 && *IMG5) {
	size += sprintf(&sql_str[size], "%sIMG6='%s'", comm, IMG5);
	strcpy(comm, ",");
    }

    if (DESTINATION_URL && *DESTINATION_URL) {
	size += sprintf(&sql_str[size], "%sDESTINATION_URL='%s'", comm, DESTINATION_URL);
	strcpy(comm, ",");
    }
    
    if (TEL && *TEL) {
	size += sprintf(&sql_str[size], "%sTEL='%s'", comm, TEL);
	strcpy(comm, ",");
    }
