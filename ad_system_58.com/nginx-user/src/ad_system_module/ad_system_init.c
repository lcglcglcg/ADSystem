...
int ad_system_init()
{
    ...
    if (lg_config_file_load(&config_file_head, "/ad_system/ad_system_58.com/conf/ad_system.conf")) {
    ...
    //发送前台使用
    if (lg_string_file_to_buffer("/etc/ad_system_region_code.json", &ad_system_global_region_code.buffer, &ad_system_global_region_code.size)) {
