...
ad_system_db_t *ad_system_db_create()
{
    ...
    db->dict_dept = ad_system_db_load_dict_dept();
...
int ad_system_db_element_add_doctors_id(lg_db_t *db,
	int doctors_id,
	ad_system_info_t *info)
{

    lg_db_node_t *doctors_id_node = lg_db_number_node_add(db, info->doctors_id);
    ad_system_list_info_t *node = ad_system_db_get_list_info();
    if (node && doctors_id_node) {

	lg_list_add(&doctors_id_node->m_list, &node->next);
	node->data = info;
    
	// fprintf(stdout, "[add][doctors_id][%d][%p][%p]\n", doctors_id, doctors_id_node, node);
    }
...
ad_system_kv_t *ad_system_db_element_find_code(char *code, int type)
{
    if (!code) return NULL;
    if (type == 1) return ad_system_db_element_find_kv(db->dict_dept, code, NULL);
...
lg_list_t *ad_system_db_element_find_img_doctors_id(int doctors_id)
{

    lg_db_node_t *node = lg_db_number_find(db->info_img_doctors_id, doctors_id);
    fprintf(stderr, "[find][doctors_id][%d][%p]\n", doctors_id, node);
    if (node) return &node->m_list;

    return NULL;
}

lg_list_t *ad_system_db_element_find_txt_doctors_id(int doctors_id)
{

    lg_db_node_t *node = lg_db_number_find(db->info_txt_doctors_id, doctors_id);
    fprintf(stderr, "[find][doctors_id][%d][%p]\n", doctors_id, node);
    if (node) return &node->m_list;
