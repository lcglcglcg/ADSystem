...
int ad_system_display_packet_parse(ngx_http_request_t *r, char *buffer, size_t size)
{
    ...
    // return lg_ngx_network_str_send(r, &network, "err");
    ...
    // get doctors_id
    int doctors_id = 0;
    node = ad_system_kv_find(&arg_head, "doctors_id");
    if (node && node->value) doctors_id = atoi(node->value);
    ...
    if (node && node->value)
	kv = ad_system_db_element_find_code(node->value, 2);
	...
	if (node && node->value)
	    kv = ad_system_db_element_find_code(node->value, 1);
    }
