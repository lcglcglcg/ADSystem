	...
	const char *account_id)
{
    ...
    char sql_str[4096] = {0};
    // sprintf(sql_str, "select count(*) from T_ACCOUNT where 1=1 %s", where_str);
	...
	const char *DOCTORS_ID)
{
    ...
    if (DOCTORS_ID && *DOCTORS_ID) {

	sql_size += sprintf(&sql_str[sql_size], "%sDOCTORS_ID=%s", comma,  DOCTORS_ID);
	comma[0] = ',';
    }
