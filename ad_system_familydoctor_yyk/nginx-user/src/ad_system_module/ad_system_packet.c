...
int ad_system_packet_get_stat_trend_text(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    char buffer[1024];
    size_t size = sprintf(buffer, "{\"CODE\":\"0\",\"ALL\":{\"IMG_BUDGET\":\"0\",\"TEXT_BUDGET\":\"0\",\"IMPRESSION\":\"0\",\"CLICK\":\"0\",\"COST\":\"0.00\"},\"TEXT\":{\"IMPRESSION\":\"0\",\"CLICK\":\"0\",\"COST\":\"0.00\"},\"IMG\":{\"IMPRESSION\":\"0\",\"CLICK\":\"0\",\"COST\":\"0.00\"}}");
    return ad_system_http_send_buffer(r, buffer, size);
}
...
int ad_system_packet_get_disease_code(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
	...
	size += sprintf(&buffer[size], "%s\"%s\":\"%s\"",
		comma, 
...
//推广->获取科室病种CODE
int ad_system_packet_get_disease_code2(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{

    ad_system_sql_t sql_conn;
    if (ad_system_sql_get_disease_code2(&sql_conn))
	return ad_system_packet_send_code(r, NULL, PACKET_CODE_ERR);
...
//推广->获取地域CODE
int ad_system_packet_get_region_code(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
...
int ad_system_packet_get_campaign(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
	...
	if (page_index >= json->page_index_s && page_index <= json->page_index_e) {
	    ...
	    max += size = lg_string_json_sprintf(buffer, comma, "}", 18,
...
int ad_system_packet_add_campaign(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    char buffer[4096];
    size_t size = sprintf(buffer, "{\"CODE\":\"0\",\"CAMPAIGN_ID\":\"%s\", \"TYPE\":\"%u\"}", campaign_id, json->TYPE);
    return ad_system_http_send_buffer(r, buffer, size);
}
...
int ad_system_packet_get_campaign_unique(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    int type = ad_system_sql_get_number(&sql_conn, NULL, 15);
    ...
    //发送数据
    char buffer[4090000];
    size_t size = lg_string_json_sprintf(buffer, "{", "}", 13,
	    "%s", "CODE", "0",
	    "%s", "CAMPAIGN_ID",          node->CAMPAIGN_ID,
	    "%s", "CAMPAIGN_NAME",        node->CAMPAIGN_NAME,
	    "%d", "CAMPAIGN_STATUS",      node->CAMPAIGN_STATE,
...
int ad_system_packet_get_group(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    char count[128];
    if (ad_system_sql_get_group_count(count,
		r->ad_system_account_id, 
		json->DISPLAY_STATUS,
		json->FIND_TYPE,
		json->CAMPAIGN_ID,
	...
	max += size = lg_string_json_sprintf(buffer, comma, "}", 15,
		...
		"%s", "CAMPAIGN_ID",        		 ad_system_sql_get_string(&sql_conn, NULL, 1),
		"%s", "CAMPAIGN_NAME",                   ad_system_sql_get_string(&sql_conn, NULL, 2),
		"%s", "GROUP_ID",                        ad_system_sql_get_string(&sql_conn, NULL, 3),
		"%s", "GROUP_NAME",                      ad_system_sql_get_string(&sql_conn, NULL, 4),
		"%s", "GROUP_STATUS",                    ad_system_sql_get_string(&sql_conn, NULL, 5),
		"%s", "GROUP_CPC_PRICE",                 ad_system_sql_get_string(&sql_conn, NULL, 6),
		"%s", "GROUP_CPM_PRICE",                 ad_system_sql_get_string(&sql_conn, NULL, 7),
		"%s", "GROUP_TARGETING_TYPE",            ad_system_sql_get_string(&sql_conn, NULL, 8),
		"%s", "GROUP_DISEASE",                   ad_system_sql_get_string(&sql_conn, NULL, 9),
		"%s", "GROUP_IMPRESSION",                "0",
		"%s", "GROUP_CLICK",                     "0",
		"%s", "GROUP_CTR",                       "0",
		"%s", "GROUP_CPC",                       "0",
		"%s", "GROUP_COST",          		 "0",
...
int  ad_system_packet_get_creative_text(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    char count[128];
    if (ad_system_sql_get_creative_text_count(count,
		r->ad_system_account_id, 
		json->FIND_TYPE, 
		json->CAMPAIGN_ID,
		json->GROUP_ID,
	...
	max += size = lg_string_json_sprintf(buffer, comma, "}", 21,
		...
		"%s", "GROUP_ID",                       ad_system_sql_get_string(&sql_conn, NULL, 3),
		"%s", "GROUP_NAME",                     ad_system_sql_get_string(&sql_conn, NULL, 4),
		"%s", "CREATIVE_ID",                    ad_system_sql_get_string(&sql_conn, NULL, 5),
		"%s", "CREATIVE_NAME",                  ad_system_sql_get_string(&sql_conn, NULL, 6),
		"%s", "CREATIVE_APPEND_NAME",           ad_system_sql_get_string(&sql_conn, NULL, 7),
		"%s", "TEL",                            ad_system_sql_get_string(&sql_conn, NULL, 8),
		"%s", "CREATIVE_DESCRIPTION",           ad_system_sql_get_string(&sql_conn, NULL, 9),
		"%s", "MAP_URL",                        ad_system_sql_get_string(&sql_conn, NULL, 10),
		"%s", "TIME_URL",                       ad_system_sql_get_string(&sql_conn, NULL, 11),
		"%s", "COMMENT_URL",                    ad_system_sql_get_string(&sql_conn, NULL, 12),
		"%s", "ADVISORY_URL",                   ad_system_sql_get_string(&sql_conn, NULL, 13),
		"%s", "MOBILE_URL",                     ad_system_sql_get_string(&sql_conn, NULL, 14),
		"%s", "DISPLAY_URL",                    ad_system_sql_get_string(&sql_conn, NULL, 15),
		"%s", "DOCTORS_NAME",                   ad_system_sql_get_string(&sql_conn, NULL, 16),
		"%s", "DOCTORS_DESCRIPTION",            ad_system_sql_get_string(&sql_conn, NULL, 17),
		"%s", "DOCTORS_IMG",                    ad_system_sql_get_string(&sql_conn, NULL, 18),
		"%s", "HOSPITAL_IMG",                   ad_system_sql_get_string(&sql_conn, NULL, 19),
		"%s", "CREATIVE_STATUS",                ad_system_sql_get_string(&sql_conn, NULL, 20),
		"%s", "HOSPITAL_LEVLE",                ad_system_sql_get_string(&sql_conn, NULL, 21)
		    );
...
int  ad_system_packet_set_creative_text(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    if (ad_system_sql_set_creative_text(
		json->CREATIVE_ID,
		json->CREATIVE_NAME,		// 创意NAME
		json->CREATIVE_APPEND_NAME,	// 附属创意NAME(创意NAME右侧)
		json->TEL,			// 电话
		json->CREATIVE_DESCRIPTION,	// 描述
		json->MAP_URL,			// 医院地址地图
		json->TIME_URL,			// 专家出诊时间
		json->COMMENT_URL,		// 网友评价
		json->ADVISORY_URL,		// 在线预约/在线咨询
		json->DISPLAY_URL,		// DISPLAY_URL
		json->MOBILE_URL,		// 移动URL
		json->DOCTORS_NAME,		// 医生NAME
		json->DOCTORS_DESCRIPTION,	// 医生描述
		json->DOCTORS_IMG,		// 医生img
		json->HOSPITAL_IMG,		// 医院img
		json->HOSPITAL_LEVLE))
	return ad_system_packet_send_code(r, NULL, PACKET_CODE_ERR);
...
int ad_system_packet_add_creative_text(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
    ...
    if (ad_system_sql_add_creative_text(r->ad_system_account_id,
		campaign_id,
		json->GROUP_ID,
		json->CREATIVE_NAME,		// 创意NAME
		json->CREATIVE_APPEND_NAME,	// 附属创意NAME(创意NAME右侧)
		...
		json->COMMENT_URL,		// 网友评价
		json->ADVISORY_URL,		// 在线预约/在线咨询
		json->DISPLAY_URL,		// display url
		json->MOBILE_URL,		// 移动URL
		json->DOCTORS_NAME,		// 医生NAME
		json->DOCTORS_DESCRIPTION,	// 医生描述
		json->DOCTORS_IMG,		// 医生img
		json->HOSPITAL_IMG,		// 医院img
		json->HOSPITAL_LEVLE))		// 医院等级
	return ad_system_packet_send_code(r, NULL, PACKET_CODE_ERR);
...
int ad_system_packet_batch_operation(ngx_http_request_t *r, ad_system_packet_json_data_t *json)
{
	...
	if (json->BATCH_TYPE == 1) {
		...
		case 2:
		    ...
		    break;
		case 3:
		    ad_system_sql_set_creative_text_status(id, json->STATUS);
		    break;
		case 4:
...
int ad_system_packet_parse(ngx_http_request_t *r)
{
	...
	if (strcmp(packet_type, "USER_LOGOUT") == 0) {
	    ...
	    // ad_system_packet_get_region_code(r, &json_data);

	} else if (strcmp(packet_type, "GET_DISEASE_CODE") == 0) {		//症状
	    ad_system_packet_get_disease_code(r, &json_data);

	} else if (strcmp(packet_type, "GET_DISEASE_CODE2") == 0) {		//症状
	    ad_system_packet_get_disease_code(r, &json_data);
