...
int ad_system_sql_init(lg_list_t *config_file_head)
{
    ...
    // setenv("ORACLE_HOME", "/oracle/10.2/", 1);
...
int ad_system_sql_connect(ad_system_sql_t *p)
{
    ...
    OCI_Connection *cn = NULL;
    if (!(cn = OCI_ConnectionCreate(oracle_host, oracle_name, oracle_pass, OCI_SESSION_DEFAULT)))
	return -1;
...
int ad_system_sql_get_disease_code(ad_system_sql_t *p)
{
    ...
    char sql_str[1024] = {0};
    sprintf(sql_str, "select name, code from t_code_39_disease order by code");
    fprintf(stdout, "%s\n", sql_str);
    return ad_system_sql_select(p, sql_str);
}

int ad_system_sql_get_disease_code2(ad_system_sql_t *p)
{

    char sql_str[1024] = {0};
    sprintf(sql_str, "select name, code from t_code_39_disease where type=2 order by code");
    return ad_system_sql_select(p, sql_str);
}


//推广->获取地域CODE
int ad_system_sql_get_region_code(ad_system_sql_t *p)
{

    char sql_str[1024] = {0};
    sprintf(sql_str, "select code, name from t_code_39_region order by code");
    return ad_system_sql_select(p, sql_str);
}
	...
	const char *keyword)
{
    ...
    OCI_Prepare(stmt, "UPDATE T_ADGROUP_TARGETING SET code = :clob WHERE ADGROUP_ID = :id");

    OCI_BindLob(stmt, ":clob", clob);
    OCI_BindString(stmt, ":id", (char *)group_id, OCI_StringUTF8Length(group_id));
    OCI_LobWrite(clob, (char *)disease, OCI_StringUTF8Length(disease));
...
//推广->获取文字创意
int ad_system_sql_get_creative_text_count(char *count,
	const char *account_id, 
	int find_type,
	const char *campaign_id, 
	const char *group_id, 
	...
	const char *find_name)
{
    ...
    char sql_str[4096] = {0};
    size_t size = sprintf(sql_str, "select "
	    "t.campaign_id,"
	    "campaign.campaign_name,"
	    "t.adgroup_id,"
	    "adgroup.adgroup_name,"
    ...
    if (campaign_id)
	size += sprintf(&sql_str[size], "and campaign_id='%s'", campaign_id);

    if (group_id)
	size += sprintf(&sql_str[size], "and adgroup_id='%s'", group_id);
...
int ad_system_sql_set_creative_text_status(char *CREATIVE_ID, int status)
{
...
//推广->修改文字创意
int ad_system_sql_set_creative_text(
	const char *CREATIVE_ID,
	const char *CREATIVE_NAME,		// 创意NAME
	const char *CREATIVE_APPEND_NAME,	// 附属创意NAME(创意NAME右侧)
	const char *TEL,			// 电话
	const char *CREATIVE_DESCRIPTION,	// 描述
	const char *MAP_URL,			// 医院地址地图
	const char *TIME_URL,			// 专家出诊时间
	const char *COMMENT_URL,		// 网友评价
	const char *ADVISORY_URL,		// 在线预约/在线咨询
	const char *DISPLAY_URL,		// DISPLAY_URL
	const char *MOBILE_URL,			// 移动URL
	const char *DOCTORS_NAME,		// 医生NAME
	const char *DOCTORS_DESCRIPTION,	// 医生描述
	const char *DOCTORS_IMG,		// 医生img
	const char *HOSPITAL_IMG,		// 医院img
	const char *HOSPITAL_LEVLE)
{


    char comma[4] = "";
    char sql_str[40960];
    size_t size = sprintf(sql_str, "update t_text_creative set ");

    if (CREATIVE_NAME && *CREATIVE_NAME) {
	size += sprintf(&sql_str[size], "%sCREATIVE_NAME='%s'", comma, CREATIVE_NAME);
	strcpy(comma, ",");
    }

    if (CREATIVE_APPEND_NAME) {
	size += sprintf(&sql_str[size], "%sCREATIVE_APPEND_NAME='%s'", comma, CREATIVE_APPEND_NAME);
	strcpy(comma, ",");
    }

    if (TEL) {
	size += sprintf(&sql_str[size], "%sTEL='%s'", comma, TEL);
	strcpy(comma, ",");
    }

    if (CREATIVE_DESCRIPTION && *CREATIVE_DESCRIPTION) {
	size += sprintf(&sql_str[size], "%sCREATIVE_DESCRIPTION='%s'", comma, CREATIVE_DESCRIPTION);
	strcpy(comma, ",");
    }

    if (MAP_URL) {
	size += sprintf(&sql_str[size], "%sMAP_URL='%s'", comma, MAP_URL);
	strcpy(comma, ",");
    }

    if (TIME_URL) {
	size += sprintf(&sql_str[size], "%sTIME_URL='%s'", comma, TIME_URL);
	strcpy(comma, ",");
    }

    if (COMMENT_URL) {
	size += sprintf(&sql_str[size], "%sCOMMENT_URL='%s'", comma, COMMENT_URL);
	strcpy(comma, ",");
    }

    if (ADVISORY_URL) {
	size += sprintf(&sql_str[size], "%sADVISORY_URL='%s'", comma, ADVISORY_URL);
	strcpy(comma, ",");
    }

    if (DISPLAY_URL && *DISPLAY_URL) {
	size += sprintf(&sql_str[size], "%sDISPLAY_URL='%s'", comma, DISPLAY_URL);
	strcpy(comma, ",");
    }

    if (MOBILE_URL) {
	size += sprintf(&sql_str[size], "%sMOBILE_URL='%s'", comma, MOBILE_URL);
	strcpy(comma, ",");
    }

    if (DOCTORS_NAME) {
	size += sprintf(&sql_str[size], "%sDOCTORS_NAME='%s'", comma, DOCTORS_NAME);
	strcpy(comma, ",");
    }

    if (DOCTORS_DESCRIPTION) {
	size += sprintf(&sql_str[size], "%sDOCTORS_DESCRIPTION='%s'", comma, DOCTORS_DESCRIPTION);
	strcpy(comma, ",");
    }

    if (DOCTORS_IMG) {
	size += sprintf(&sql_str[size], "%sDOCTORS_IMG='%s'", comma, DOCTORS_IMG);
	strcpy(comma, ",");
    }

    if (HOSPITAL_IMG) {
	size += sprintf(&sql_str[size], "%sHOSPITAL_IMG='%s'", comma, HOSPITAL_IMG);
	strcpy(comma, ",");
    }
    
    if (HOSPITAL_LEVLE) {
	size += sprintf(&sql_str[size], "%sHOSPITAL_LEVLE=%s", comma, HOSPITAL_LEVLE);
	strcpy(comma, ",");
    }

    sprintf(&sql_str[size], "where CREATIVE_ID='%s'", CREATIVE_ID);
    fprintf(stderr, "==>%s\n", sql_str);
...
//推广->添加文字创意
int ad_system_sql_add_creative_text(
	const char *account_id,
	const char *campaign_id,
	const char *group_id,
	const char *CREATIVE_NAME,		// 创意NAME
	...
	const char *COMMENT_URL,		// 网友评价
	const char *ADVISORY_URL,		// 在线预约/在线咨询
	const char *DISPLAY_URL,		// display url
	const char *MOBILE_URL,			// 移动URL
	const char *DOCTORS_NAME,		// 医生NAME
	const char *DOCTORS_DESCRIPTION,	// 医生描述
	const char *DOCTORS_IMG,		// 医生img
	const char *HOSPITAL_IMG,		// 医院img
	const char *HOSPITAL_LEVLE)		// 医院等级
{


    char sql_str[40960];
    sprintf(sql_str, "insert into t_text_creative ("
	    "ACCOUNT_ID,"
	    "CAMPAIGN_ID,"
	    "ADGROUP_ID,"
	    "CREATIVE_ID,"
	    "CREATIVE_NAME,"
	    "CREATIVE_APPEND_NAME,"
	    "TEL,"
	    ...
	    "'%s',"		// campaign_id
	    "'%s',"		// group_id
	    "SEQ_T_TEXT_CREATIVE.Nextval,"	//CREATIVE_ID
	    "'%s',"		// 创意NAME
	    "'%s',"              // 附属创意NAME(创意NAME右侧)
	    "'%s',"              // 电话
	    "'%s',"              // 描述
	...
	CREATIVE_APPEND_NAME,	// 附属创意NAME(创意NAME右侧)
	TEL,			// 电话
	CREATIVE_DESCRIPTION,	// 描述
	MAP_URL,		// 医院地址地图
	TIME_URL,		// 专家出诊时间
	COMMENT_URL,		// 网友评价
	ADVISORY_URL,		// 在线预约/在线咨询
	DISPLAY_URL,		// DISPLAY_URL
	MOBILE_URL,		// 移动URL
	DOCTORS_NAME,		// 医生NAME
	DOCTORS_DESCRIPTION,	// 医生描述
	DOCTORS_IMG,		// 医生img
	...
	const char *img_path)
{
    ...
    char IMG_PATH[1024] = "IMG_PATH";
    if (img_path && *img_path)
	sprintf(IMG_PATH, "'%s'", img_path);
	...
	const char *keyword)
{
    ...
    char sql_str[1024] = {0}; 
    sprintf(sql_str, "INSERT INTO T_ADGROUP_TARGETING (ADGROUP_TARGETING_ID,ACCOUNT_ID,CAMPAIGN_ID,ADGROUP_ID,CREATE_TIME,LAST_UPDATE_TIME) VALUES (SEQ_T_ADGROUP_TARGETING.nextval,%s,'%s','%s',sysdate,sysdate)",
	    account_id,
	    campaign_id,
	    group_id);
    ...
    ad_system_sql_set_group_targeting(
	    account_id,
	    group_id,
	    0,
	    0,
...
int _ad_system_sql_del_creative_img(
	const char *campaign_id, 
	const char *group_id, 
	const char *creative_id)
{
    ...
    char sql_str[4096];
    if (creative_id)
	sprintf(sql_str, "delete from t_creative where creative_id='%s'", creative_id);
    else if (group_id)
	sprintf(sql_str, "delete from t_creative where adgroup_id='%s'", group_id);
    else if (campaign_id)
	sprintf(sql_str, "delete from t_creative where campaign_id='%s'", campaign_id);
...
int _ad_system_sql_del_creative_text(
	const char *campaign_id, 
	const char *group_id, 
	const char *creative_id)
{
    ...
    char sql_str[4096];
    if (creative_id)
	sprintf(sql_str, "delete from t_text_creative where creative_id='%s'", creative_id);
    else if (group_id)
	sprintf(sql_str, "delete from t_text_creative where adgroup_id='%s'", group_id);
    else if (campaign_id)
	sprintf(sql_str, "delete from t_text_creative where campaign_id='%s'", campaign_id);
...
int _ad_system_sql_del_group(
	const char *campaign_id, 
	const char *group_id)
{
    char sql_str[4096];
    if (group_id)
	sprintf(sql_str, "delete from t_adgroup where adgroup_id='%s'", group_id);
    else if (campaign_id)
	sprintf(sql_str, "delete from t_adgroup where campaign_id='%s'", campaign_id);
...
int _ad_system_sql_del_campaign(const char *campaign_id)
{
    char sql_str[4096];
    sprintf(sql_str, "delete from t_campaign where campaign_id='%s'", campaign_id);
    fprintf(stderr, "==>%s\n", sql_str);
...
int ad_system_sql_del_campaign(const char *campaign_id)
{
    ...
    _ad_system_sql_del_creative_img(campaign_id, NULL, NULL);
    _ad_system_sql_del_creative_text(campaign_id, NULL, NULL);
    _ad_system_sql_del_group(campaign_id, NULL);
    return _ad_system_sql_del_campaign(campaign_id);
}

//删除组
int ad_system_sql_del_group(const char *group_id)
{
    _ad_system_sql_del_creative_img(NULL, group_id, NULL);
    _ad_system_sql_del_creative_text(NULL, group_id, NULL);
    return _ad_system_sql_del_group(NULL, group_id);
}

//删除图片创意
int ad_system_sql_del_creative_img(const char *creative_id)
{
    return _ad_system_sql_del_creative_img(NULL, NULL, creative_id);
}
...
int ad_system_sql_del_creative_text(const char *creative_id)
{
    ...
    return _ad_system_sql_del_creative_text(NULL, NULL, creative_id);

    char sql_str[4096];
    sprintf(sql_str, "delete t_text_creative from creative_id='%s'", creative_id);
    fprintf(stderr, "==>%s\n", sql_str);
...
int ad_system_sql_get_group_count(char *count,
	const char *account_id, 
	int display_status, 
	int find_type,
	const char *campaign_id, 
