...
int ad_system_db_select_cmp_r(ngx_queue_t *head, int level, ad_system_info_t *info)
{
	...
	if (level == 0 || info->level == 0 || level == info->level) {
	...
	ngx_queue_t *place)
{
	...
	ad_system_db_select_cmp_r(place, level, info);
    }

    // 剩余流量
    lg_list_t *default_head = ad_system_db_element_find_txt_default();
    lg_list_for_each(p, default_head) {

	ad_system_info_t *info = lg_list_entry(p, ad_system_info_t, select_next);
	
	fprintf(stderr, "[%s]\n", info->creative_id);
	ad_system_db_select_cmp_r(place, 0, info);
    }
