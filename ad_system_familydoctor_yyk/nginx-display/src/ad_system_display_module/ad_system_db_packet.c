...
int ad_system_display_packet_parse(ngx_http_request_t *r, char *buffer, size_t size)
{
    ...
    // get level
    int level = 0;
    node = ad_system_kv_find(&arg_head, "level");
    if (node && node->value) level = atoi(node->value);

    //get disease code
    ad_system_kv_t *kv = NULL;
    char *disease_code = NULL;

    // 取疾病
    node = ad_system_kv_find(&arg_head, "disease");
    if (node && node->value && *node->value) 
	kv = ad_system_db_element_find_disease(node->value);

    if (!kv) {

	// 查科室
	node = ad_system_kv_find(&arg_head, "dept");
	if (node && node->value)
	    kv = ad_system_db_element_find_dept(node->value);
    }

    if (!kv) {

	// 专科医院
	node = ad_system_kv_find(&arg_head, "special");
	if (node && node->value && *node->value) 
	    kv = ad_system_db_element_find_special(node->value);
    }

    if (kv && *kv->value) disease_code = kv->value;


    // get region
    kv = NULL;
    node = ad_system_kv_find(&arg_head, "region");
    if (node && node->value && *node->value)
	kv = ad_system_db_element_find_region(node->value);

    char *region_code = NULL;
    if (kv && kv->value && *kv->value) region_code = kv->value;
