...
ad_system_db_t *ad_system_db_create()
{
    ...
    db->dict_special = ad_system_db_load_dict_special();
...
void ad_system_db_free(ad_system_db_t *db)
{
    ...
    lg_db_free(db->dict_dept, ad_system_db_data_free);
    lg_db_free(db->dict_disease, ad_system_db_data_free);

    // 剩余流量
    lg_list_t *p = NULL;
    lg_list_for_each(p, &db->info_txt_default) {
...
ad_system_kv_t *ad_system_db_element_find_dept(char *code)
{
    return ad_system_db_element_find_kv(db->dict_dept, code, NULL);
}
...
ad_system_kv_t *ad_system_db_element_find_special(char *code)
{
    return ad_system_db_element_find_kv(db->dict_special, code, NULL);
}
...
ad_system_kv_t *ad_system_db_element_find_region(char *code)
{
    if (!code) return NULL;
    return ad_system_db_element_find_kv(db->dict_region, code, NULL);
}
...
lg_list_t *ad_system_db_element_find_txt_default()
{
    return &db->info_txt_default;
}
