	...
	ad_system_merger_campaign_info_t *info)
{
	...
	temp.type = ad_system_sql_get_number(sql_conn, NULL, 15);
	...
	ad_system_merger_group_info_t *info)
{
	...
	fprintf(stderr, "GROUP_STATE==[1]%d\n", GROUP_STATE);
	...
	fprintf(stderr, "GROUP_STATE==[2]%d\n", temp.GROUP_STATE);
	...
	if ((node = malloc(sizeof(ad_system_merger_group_t)))) {
	    ...
	    fprintf(stderr, "GROUP_STATE==[3]%d\n", node->GROUP_STATE);
	...
	ad_system_merger_creative_info_t *info)
{
	...
	sprintf(temp.IMG_PATH, "%s", ad_system_sql_get_string(sql_conn, NULL, 16));
	temp.type = ad_system_sql_get_number(sql_conn, NULL, 17);
